...
export function SettingsPage() {
  ...
  return (
    <div className="h-screen w-full bg-slate-950 text-slate-200 font-sans flex flex-col relative overflow-hidden">
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto p-4 lg:p-8 flex flex-col lg:flex-row gap-8 relative z-20 overflow-hidden">
        {/* Desktop Sidebar Navigation */}
        <nav className="hidden lg:block w-64 shrink-0 space-y-2 h-full overflow-y-auto pb-4">
          {tabs.map((tab) => {
            const Icon = tab.icon;
            const isActive = activeTab === tab.id;
            return (
        ...
        {/* Content Area */}
        <div className="flex-1 min-w-0 h-full overflow-y-auto pb-20 px-1">
          <motion.div
            key={activeTab}
          ...
          >
            ...
            {activeTab === 'general' && (
              <div className="space-y-6">
                <section className="bg-slate-900/40 border border-white/10 rounded-2xl p-6 backdrop-blur-md">
